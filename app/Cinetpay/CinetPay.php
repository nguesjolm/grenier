...
class CinetPay
{
    ...
    public static function generateTransId()
    {
        ...
        $id = sprintf('%06d', $id) . mt_rand(100, 99999);
    ...
    public function getPayButton($formName, $btnType = 1, $size = "large")
    {
        ...
        $form = "<form style='text-align: center;' id='" . $formName . "' name='" . $formName . "' action='" . $this->_cashDeskUri . "' class='cinetpay-button' method='post'>";
    ...
    public function getOnlyPayButtonToSubmit($formName, $btnType = 1, $size)
    {
        ...
        if (!empty($formName) && $btnType == 1) {
            ...
            $btn = "<button class='btn btn-primary " . $size . "' onclick='javascript:document.forms['" . $formName . "'].submit()'> Confirmer le payement </button>";
        }
        return $btn;
    }
    ...
    public function isAuthentified()
    {
        if ($this->getPayStatus()) {
            ...
            if (!empty($this->_cpm_custom)) {
                $dataArray['cpm_custom'] = $this->_cpm_custom;
            }
